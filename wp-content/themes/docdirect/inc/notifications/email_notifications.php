    ...
    class DocDirectProcessEmail
    {
        ...
        public function getCalenderLinks($title, $from, $to, $description, $address)
        {
            ...
            <ul style="display:block;float:left;width:100%; padding:0; margin:0;">
                <li style="display:block;float:left;width:100%; padding:0; margin:0;"><a
                            style="font-family: 'Helvetica Neue', 'Helvetica', Helvetica, Arial, sans-serif; font-size: 100%; line-height: 1.6em; color: #348eda; margin: 0; padding: 0;"
                            href="<?php echo($google_url); ?>"><?php esc_html_e('Add to Google Calender', 'docdirect'); ?></a>
                </li>
                <li style="display:block;float:left;width:100%; padding:0; margin:0;"><a
                            style="font-family: 'Helvetica Neue', 'Helvetica', Helvetica, Arial, sans-serif; font-size: 100%; line-height: 1.6em; color: #348eda; margin: 0; padding: 0;"
                            href="<?php echo($yahoo_url); ?>"><?php esc_html_e('Add to Yahoo Calender', 'docdirect'); ?></a>
                </li>
                <li style="display:block;float:left;width:100%; padding:0; margin:0; margin-bottom: 15px;"><a
                            style="font-family: 'Helvetica Neue', 'Helvetica', Helvetica, Arial, sans-serif; font-size: 100%; line-height: 1.6em; color: #348eda; margin: 0; padding: 0;"
                            href="<?php echo($ical_url); ?>"><?php esc_html_e('Add to Outlook', 'docdirect'); ?></a>
                </li>
            </ul>
            <?php
            return ob_get_clean();
        ...
        public function prepare_email_footers($params = '')
        {
            ...
            <p style="margin: 0;width:100%;float:left;background: #002c49;padding: 30px 15px;text-align:center;box-sizing:border-box;border-radius: 0  0 5px 5px;color:#FFF;">
                &copy;<?php echo date('Y'); ?><?php esc_html_e(' | All Rights Reserved', 'docdirect'); ?> <a
                        href="<?php echo esc_url(home_url('/')); ?>"
                        style="color:#FFF;font-family: 'Helvetica Neue', 'Helvetica', Helvetica, Arial, sans-serif; font-size: 100%; line-height: 1.6em; color: #348eda; margin: 0; padding: 0;"><?php echo esc_attr($blogname); ?></a>
            </p></td>
            </tr>
            </table>
            </div>
         ...
         * @since 1.0.0
         */
        public function process_new_inbox_message($params = '')
        {
            global $current_user;
            extract($params);

            $user__name = docdirect_get_username($user_name);
            $from__user_name = docdirect_get_username($from_user_name);

            $subject_default = esc_html__('A new message has received!', 'docdirect');
            $email_content_default = 'Hi %user_name%!<br/>
									 You have received a new messages from %from_user_name%<br/>
            ...
            if (function_exists('fw_get_db_post_option')) {
                $new_messages = fw_get_db_settings_option('new_messages');
                if (!empty($new_messages['gadget']) && $new_messages['gadget'] === 'enable') {
                    $privacy = docdirect_get_privacy_settings($user_name); //Privacy settings
                    if (!empty($privacy['chat_message']) && $privacy['chat_message'] === 'on') {
                        $messages_enbable = 'yes';
                    } else {
                        $messages_enbable = 'no';
                    }

                    $subject = !empty($new_messages['enable']['message_subject']) ? $new_messages['enable']['message_subject'] : '';
                    $email_content = !empty($new_messages['enable']['messages_content']) ? $new_messages['enable']['messages_content'] : '';
                }

            }

            if (!empty($messages_enbable) && $messages_enbable === 'yes') {
                $user_to_data = get_userdata($user_name);
                $email = !empty($user_to_data->user_email) ? $user_to_data->user_email : '';
                ...
                $email_content = str_replace("%user_name%", $user__name, $email_content);
                $email_content = str_replace("%from_user_name%", $from__user_name, $email_content);
                $email_content = str_replace("%message%", $message, $email_content);
                $email_content = str_replace("%logo%", nl2br($logo), $email_content);
