    ...
    function docdirect_edit_user_profile_edit($user_identity)
    {
        ...
        <table class="form-table">
            ...
            <tr>
                ...
                <td>
                    <input type="hidden" name="userprofile_media" class="media-image" id="userprofile_media"
                           value="<?php echo intval($thumb_id); ?>"/>
                    <input type="button" id="upload-user-avatar" class="button button-secondary"
                           value="<?php esc_html_e('Uplaod Public Avatar', 'docdirect'); ?>"/>
                </td>
            </tr>
            <tr id="avatar-wrap" class="<?php echo esc_attr($display_image); ?>">
                <td class="backgroud-image">
                    ...
                    <img class="avatar-src-style" height="100px" src="<?php echo esc_url($display_img_url); ?>"
                         id="avatar-src"/>
        ...
        <table class="form-table">
            ...
            <tr>
                ...
                <td>
                    <input type="hidden" name="userprofile_banner" class="media-image" id="userprofile_banner"
                           value="<?php echo intval($banner_thumb_id); ?>"/>
                    <input type="button" id="upload-user-banner" class="button button-secondary"
                           value="<?php esc_html_e('Profile banner', 'docdirect'); ?>"/>
                </td>
            </tr>
            <tr id="banner-wrap" class="<?php echo esc_attr($display_banner); ?>">
                <td class="backgroud-image">
                    ...
                    <img class="banner-src-style" height="100px" src="<?php echo esc_url($display_banner_url); ?>"
                         id="banner-src"/>
    ...
    function docdirect_personal_options_save($user_identity)
    {
        ...
        if (empty($updated_pack_id)) { //Update only featurd and package expiry date
            ...
            if (!empty($_POST['featured_days'])) {
                ...
                if (!empty($user_featured_date['featured_till']) && $user_featured_date['featured_till'] > $current_date) {
                ...
                if (strtotime($current_date) > strtotime($featured_date)) {
                    update_user_meta($user_identity, 'user_featured', 1);
                    update_user_meta($user_identity, 'user_featured_expiry', strtotime($featured_date));
                } else {
                    update_user_meta($user_identity, 'user_featured', 0);
                }
                ...
                if (isset($user_featured_date['featured_till']) && !empty($user_featured_date['featured_till'])) {
                    $featured_date = strtotime("-" . $duration . " days", $user_featured_date['featured_till']);
                    $featured_date = date('Y-m-d H:i:s', $featured_date);
                }
            ...
            if (apply_filters('docdirect_get_theme_settings', 'payments') === 'woo') { //If woocomerce is enabled
                ...
                if (apply_filters('docdirect_get_packages_setting', 'default') === 'custom') {
                    ...
                    //Featured
                    if (!empty($user_featured_date['featured_till']) && $user_featured_date['featured_till'] > $payment_date) {
                ...
                $current_date = date('Y-m-d H:i:s');
                if (strtotime($current_date) > $featured_date) {
                    update_user_meta($user_identity, 'user_featured', 1);
                    update_user_meta($user_identity, 'user_featured_expiry', $featured_date);
                } else {
                    update_user_meta($user_identity, 'user_featured', 0);
                }
        ...
        if (!empty($_POST['awards'])) {
            ...
            foreach ($_POST['awards'] as $key => $value) {
                ...
                $awards[$counter]['date_formated'] = date('d M, Y', strtotime(esc_attr($value['date'])));
        ...
        if (!empty($_POST['education'])) {
            ...
            foreach ($_POST['education'] as $key => $value) {
                ...
                $educations[$counter]['start_date_formated'] = date('M,Y', strtotime(esc_attr($value['start_date'])));
                $educations[$counter]['end_date_formated'] = date('M,Y', strtotime(esc_attr($value['end_date'])));
        ...
        if (!empty($_POST['experience'])) {
            ...
            foreach ($_POST['experience'] as $key => $value) {
                if (!empty($value['title'])) {
                    ...
                    $experiences[$counter]['start_date_formated'] = date('M,Y', strtotime(esc_attr($value['start_date'])));
                    $experiences[$counter]['end_date_formated'] = date('M,Y', strtotime(esc_attr($value['end_date'])));
    ...
    function docdirect_import_users()
    {
        ...
        <div id="import-users" class="import-users">
            <div class="theme-screenshot">
                <img alt="<?php esc_attr_e('Import Users', 'docdirect'); ?>"
                     src="<?php echo get_template_directory_uri(); ?>/core/images/users.jpg">
            </div>
            <h3 class="theme-name"><?php esc_html_e('Import Users', 'docdirect'); ?></h3>
            <div class="user-actions">
                <a href="javascript:;"
                   class="button button-primary doc-import-users"><?php esc_html_e('Import', 'docdirect'); ?></a>
            </div>
        </div>
        <div id="import-users" class="import-users custom-import">
            <form method="post" action="<?php echo cus_prepare_final_url('file', 'import_users'); ?>"
                ...
                <h3 class="theme-name">
                    ...
                    <label for="upload-dummy-csv"
                           class="button button-primary upload-dummy-csv"><?php esc_html_e('Choose File', 'docdirect'); ?></lable>
                </h3>
                <div class="user-actions">
                    <input type="submit" class="button button-primary"
                           value="<?php esc_html_e('Import From File', 'docdirect'); ?>">
                        ...
                        <div class="tg-checkbox user-selection">
                            <div class="tg-packages active-user-type specialities-type">
                                <input type="checkbox" <?php echo esc_attr($checked); ?>
                                       name="specialities[<?php echo esc_attr($speciality->term_id); ?>]"
                                       value="<?php echo esc_attr($speciality->slug); ?>"
                                       id="<?php echo esc_attr($speciality->slug); ?>">
