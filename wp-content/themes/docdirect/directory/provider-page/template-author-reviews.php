    ...
    <?php if (isset($reviews_switch) && $reviews_switch === 'enable') { ?>
        <div class="tg-userreviews">
            <div class="tg-userheading">
                <h2><?php echo intval(apply_filters('docdirect_count_reviews', $author_profile->ID)); ?>
                    &nbsp;&nbsp;<?php esc_html_e('Review(s)', 'docdirect'); ?></h2>
            </div>
                            ...
                            <li>
                                <div class="tg-review">
                                    <figure class="tg-reviewimg">
                                        <a href="<?php echo get_author_posts_url($user_from); ?>"><img
                                                    src="<?php echo esc_url($avatar); ?>"
                                                    alt="<?php esc_html_e('Reviewer', 'docdirect'); ?>"></a>
                                    </figure>
                                    <div class="tg-reviewcontet">
                                        <div class="tg-reviewhead">
                                            <div class="tg-reviewheadleft">
                                                <h3>
                                                    <a href="<?php echo get_author_posts_url($user_from); ?>"><?php echo esc_attr($user_name); ?></a>
                                                </h3>
                                                <span><?php echo human_time_diff(strtotime($review_date), current_time('timestamp')) . '&nbsp;' . esc_html__('ago', 'docdirect'); ?></span>
                                            </div>
                                ...
                                <div class="row">
                                    <div class="col-sm-6">
                                        <div class="form-group">
                                            <input type="text" name="user_subject" class="form-control"
                                                   placeholder="<?php esc_attr_e('Subject', 'docdirect'); ?>">
                                        </div>
                                    </div>
                                    <div class="col-sm-6">
                                        <div class="tg-stars">
                                            <div id="jRate"></div>
                                            <span class="your-rate"><strong><?php esc_html_e('Excellent', 'docdirect'); ?></strong></span>
                                        </div>
                                    ...
                                    <div class="col-sm-12">
                                        <div class="form-group">
                                            <textarea class="form-control" name="user_description"
                                                      placeholder="<?php esc_attr_e('Review Description *', 'docdirect'); ?>"></textarea>
                                        </div>
                                    </div>
                                    <div class="col-sm-12">
                                        <button class="tg-btn make-review"
                                                type="submit"><?php esc_html_e('Submit Review', 'docdirect'); ?></button>
                                        <input type="hidden" name="user_rating" class="user_rating" value=""/>
                                        <input type="hidden" name="user_to" class="user_to"
                                               value="<?php echo esc_attr($author_profile->ID); ?>"/>
                                    </div>
                                </div>
                            </fieldset>
                        </form>
                    <?php } else { ?>
                        <span><a href="javascript:;" class="tg-btn" data-toggle="modal"
                                 data-target=".tg-user-modal"><?php esc_html_e('Please Login To add Review', 'docdirect'); ?></a></span>
                    <?php } ?>
                </div>
            <?php } ?>
        </div>
