...
<div class="tg-usercontactinfo">
    ...
    <ul class="tg-doccontactinfo">
        ...
        <?php if (!empty($author_profile->user_email)
            ...
            <li><i class="fa fa-envelope-o"></i><a
                        href="mailto:<?php echo esc_attr($author_profile->user_email); ?>?subject:<?php esc_html_e('Hello', 'docdirect'); ?>"><?php echo esc_attr($author_profile->user_email); ?></a>
            </li>
        <?php } ?>
        <?php if (!empty($author_profile->phone_number)
            ...
            <li><i class="fa fa-phone"></i> <span><a
                            href="tel:<?php echo esc_attr($author_profile->phone_number); ?>"><?php echo esc_attr($author_profile->phone_number); ?></a></span>
            </li>
        <?php } ?>
        ...
        <?php if (!empty($author_profile->user_url)) { ?>
            <li><i class="fa fa-link"></i><a href="<?php echo esc_url($author_profile->user_url); ?>"
    ...
    <?php if (!empty($social_links)) { ?>
        ...
        <ul class="tg-socialicon-v2">
            ...
            foreach ($social_links as $key => $social) {
                ...
                if (!empty($item)) { ?>
                    <li class="<?php echo esc_attr($classes); ?>"><a href="<?php echo esc_attr($item); ?>"
                                                                     style="background:<?php echo esc_attr($color); ?>"><i
    ...
    <?php } ?>
    <?php if (!empty($author_profile->user_address)) { ?>
        <a class="tg-btn tg-btn-lg"
           href="http://maps.google.com/maps?saddr=&amp;daddr=<?php echo esc_attr($author_profile->user_address); ?>"
           target="_blank"><?php esc_html_e('get directions', 'docdirect'); ?></a>
    <?php } ?>

    <?php
    if (is_user_logged_in()) {
        if ($current_user->ID != $author_profile->ID) {
            if (!empty($is_chat) && $is_chat === 'yes' && $directory_type != 123) {
                ?>
                <a class="tg-btn tg-btn-lg send-message" data-toggle="modal" data-target="#chatmodal"
                   href="javascript:;"><i
                            class="lnr lnr-bubble"></i><?php esc_html_e('&nbsp;Send message', 'docdirect'); ?></a>
                <?php
            }
        }
    }
