    ...
    function docdirect_get_custom_slots($data = '', $return_type = "return")
    {
        ...
        if (!empty($data['timeslot'])) {
            foreach ($data['timeslot'] as $key => $value) {
                ...
                <div class="tg-doctimeslot form-group">
                    <div class="tg-box">
                        <a class="tg-deleteslot delete-custom-slot" data-time="<?php echo esc_attr($key); ?>"
                           href="javascript:;"><i class="fa fa-close"></i></a>
            ...
            <div class="form-group">
                <div class="doc-select">
                    <select name="bk_service" class="bk_service">
                        ...
                        if (!empty($booking_services)) {
                            ...
                            foreach ($booking_services as $key => $value) {
                                ...
                                ?>
                                <option value="<?php echo esc_attr($key); ?>"><?php echo esc_attr($value['title']); ?>
                                    &nbsp;--&nbsp;<?php echo esc_attr($currency_symbol); ?><?php echo esc_attr($value['price']); ?></option>
                                <?php
                            }
                        }
                        ?>
                    ...
                    <script type="text/template" id="tmpl-load-services">
                        ...
                        <option value="{{index}}">
                            {{element.title}}&nbsp;--&nbsp;<?php echo esc_attr($currency_symbol); ?>{{element.price}}
                        </option>
                        <#
                        });
                        }
...
if (!function_exists('docdirect_get_booking_step_two')) {
    ...
    function docdirect_get_booking_step_two()
    {
        ...
        ob_start();
        if ($todays_defaults == false) {
            $json['data'] = '<h3>No available schedule found</h3>';
            $json['type'] = 'success';
            $json['message'] = esc_html__('slots returned with null', 'docdirect');
            ...
            <div class="tg-appointmenttime">
                <?php if (!empty($schedule_message)) { ?>
                    <div class="tg-description">
                        <p style="font-weight: 700;">Select Date and Schedule</p>
                    </div>
                <?php } ?>
...
if (!function_exists('docdirect_get_booking_step_three')) {
    function docdirect_get_booking_step_three()
    {
        ...
        <div class="bk-customer-form">
            ...
            <div class="form-group">
                <input type="text" class="form-control" name="username" value="<?php echo esc_attr($username); ?>"
                       placeholder="<?php esc_attr_e('Your Name*', 'docdirect'); ?>">
            </div>
            <div class="form-group">
                <input type="text" class="form-control" value="<?php echo esc_attr($userphone); ?>" name="userphone"
                       placeholder="<?php esc_attr_e('Phone*', 'docdirect'); ?>">
            </div>
            <div class="form-group">
                <input type="text" class="form-control" name="useremail" value="<?php echo esc_attr($useremail); ?>"
                       placeholder="<?php esc_attr_e('Email*', 'docdirect'); ?>">
            </div>
            <div class="form-group tg-textarea">
                <textarea class="form-control" name="booking_note"
                          placeholder="<?php esc_attr_e('Note', 'docdirect'); ?>"></textarea>
            </div>
        </div>
        <?php
        $json['data'] = ob_get_clean();
