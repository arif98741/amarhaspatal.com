...
if (!function_exists('docdir_update_booking_settings')) {
    function docdir_update_booking_settings()
    {
        ...
        update_user_meta($user_identity, 'currency', sanitize_text_field('BDT'));
        //update_user_meta( $user_identity, 'currency_symbol', sanitize_text_field( $_POST['currency_symbol'] ) );
        update_user_meta($user_identity, 'currency_symbol', sanitize_text_field('৳'));
...
if (!function_exists('docdirect_locateme_snipt')) {
    function docdirect_locateme_snipt()
    {
        ...
        <div class="locate-me-wrap">
            ...
            <input type="text" autocomplete="on" id="location-address" value="<?php echo esc_attr($location); ?>"
                   name="geo_location" placeholder="<?php esc_html_e('Geo location', 'docdirect'); ?>"
                   class="form-control">
            <?php if (isset($dir_geo) && $dir_geo === 'enable') { ?>
                <a href="javascript:;" class="geolocate"><img
                            src="<?php echo get_template_directory_uri(); ?>/images/geoicon.svg" width="16" height="16"
                            class="geo-locate-me" alt="<?php esc_html_e('Locate me!', 'docdirect'); ?>"></a>
            <?php } ?>
            <?php if (isset($dir_radius) && $dir_radius === 'enable') { ?>
                ...
                <div class="geodistance_range elm-display-none">
                    <div class="distance-ml"><?php esc_html_e('Distance in', 'docdirect'); ?>
                        &nbsp;<?php echo esc_attr($distance_title); ?><span><?php echo esc_js($distance); ?></span>
                    </div>
                    <input type="hidden" name="geo_distance" value="<?php echo esc_js($distance); ?>"
                           class="geo_distance"/>
...
if (!function_exists('docdirect_get_featured_tag')) {
    function docdirect_get_featured_tag($echo = false, $view = 'v1')
    {
        ...
        if (isset($view) && $view === 'v2') {
            ?>
            <a class="doc-featuredicon" href="javascript:;"><i
                        class="fa fa-bolt"></i><?php esc_html_e('featured', 'docdirect'); ?></a>
            <?php
        } else {
            ?>
    ...
    function docdirect_get_verified_tag($echo = false, $user_id = '', $view_type = 'svg', $view = 'v1')
    {
        ...
        if (!empty($user_id)) {
            ...
            if (isset($featured_date) && $featured_date === 'on') {
                ...
                if (isset($view) && $view === 'v2') {
                    ?>
                    <a class="doc-featuredicon doc-verfiedicon" href="javascript:;"><i
                                class="fa fa-shield"></i><?php esc_html_e('Verified', 'docdirect'); ?></a>
                    <?php
                } else {
                    if (isset($view_type) && $view_type === 'simple') {
                        ?>
                        <li class="tg-varified"><a href="javascript:;"><i
                                        class="fa fa-shield"></i><span><?php esc_html_e('Verified', 'docdirect'); ?></span></a>
                        </li>
                    ...
                    <form class="tg-form-modal tg-form-signup do-forgot-form">
                        <fieldset>
                            <div class="form-group">
                                ...
                                <p><?php esc_html_e('Forgot your password? Enter the email address for your account to reset your password, otherwise you can', 'docdirect'); ?>
                                    <a href="javascript:;"
                                       class="try-again-lp">&nbsp;<?php esc_html_e('try again', 'docdirect'); ?></a></p>
                                <div class="forgot-fields">
                                    <div class="form-group">
                                        <input type="email" name="psemail" class="form-control psemail"
                                               placeholder="<?php esc_html_e('Email Address*', 'docdirect'); ?>">
                                        <input type="hidden" name="tg_pwd_nonce"
                                               value="<?php echo wp_create_nonce("tg_pwd_nonce"); ?>"/>
                                    </div>
                                </div>
                                <button class="tg-btn tg-btn-lg  do-lp-button"
                                        type="button"><?php esc_html_e('Submit', 'docdirect'); ?></button>
                            </div>
                        </fieldset>
                    </form>
                </div>
        ...
        if (!empty($_GET['key']) &&
            ...
            if ($reset_key === $key) {
                ...
                if (!empty($user_data)) {
                                ...
                                <form class="tg-form-modal tg-form-signup do-reset-form">
                                    <fieldset>
                                        <div class="form-group">
                                            ...
                                            <button class="tg-btn tg-btn-lg  do-reset-button"
                                                    type="button"><?php esc_html_e('Submit', 'docdirect'); ?></button>

                                            <input type="hidden" name="key"
                                                   value="<?php echo esc_attr($reset_key); ?>"/>
                                            <input type="hidden" name="reset_action"
                                                   value="<?php echo esc_attr($reset_action); ?>"/>
                    ...
                    <a href="javascript:;" class="open-reset-window" data-toggle="modal"
                       data-target=".tg-user-reset-model"></a>
                    <?php
                    echo ob_get_clean();
        ...
        if (!empty($terms)) {
            ...
            <div class="tg-posttags">
                ...
                <?php foreach ($terms as $key => $terms) { ?>
                    <a class="tg-tag <?php echo esc_attr($classes); ?>"
                       href="<?php echo get_term_link($terms->term_id, $categoty_type); ?>"><?php echo esc_attr($terms->name); ?></a>
                <?php } ?>
            ...
            <div class="modal-dialog tg-modaldialog" role="document">
                <div class="modal-content tg-modalcontent tg-add-questions">
                            ...
                            <fieldset>
                                <div class="form-group">
                                    <input type="text" class="question_title" name="question_title"
                                           placeholder="<?php esc_html_e('Question title', 'docdirect'); ?>">
                                </div>
                                <div class="form-group">
                                ...
                                <div class="form-group">
                                    <textarea class="question_details" name="question_description"
                                              placeholder="<?php esc_html_e('Ask question here', 'docdirect'); ?>"></textarea>
                    ...
                    <div class="tg-modalfoot">
                        <a href="javascript:;" class="tg-btn fw_ext_question_save_btn" data-type="open"
                           type="submit"><?php esc_html_e('Ask a Question', 'docdirect'); ?></a>
                    </div>
                </div>
            </div>
        </div>
